...
noaa %>% 
  ...
  summarise(avg_max_temp = mean(tmax_c, na.rm = TRUE)) %>% 
...
```
    ...
    ## Warning: Removed 5979 rows containing non-finite values (stat_smooth).

    ## Warning: Removed 5979 rows containing missing values (geom_point).

<img src="p8105_hw3_js5431_files/figure-markdown_github/two panel plot max temp-1.png" width="90%" />
