...
The Instacart dataset includes data from selected orders from an online grocery store; it contains rows and columns, and is formatted such that each row is a product from an order. Only one order for any particular individual is inlcuded.
